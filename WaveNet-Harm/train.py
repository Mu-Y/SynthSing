"""Training script for the WaveNet network on the HIT Japanese Nursery corpus.
...
def main():
    ...
    # pdb.set_trace()
    loss = net.loss(input_batch=ap_batch,                                # audio_batch shape: [receptive_filed + sample_size, 1]
                    lc_batch = lc_batch,
    ...
    print ("========================================")
    print ("Total number of parameteres for mfsc model:", np.sum([np.prod(v.get_shape().as_list()) for v in tf.trainable_variables()]))
    # pdb.set_trace()
    step = None
